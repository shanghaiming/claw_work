...
def get_actual_date_range():
    ...
    try:
        # 加载一个示例文件获取日期范围
        example_file = '/Users/chengming/.openclaw/workspace/quant_integration/phase4_system_integration/data/example_data/TEST001.SZ.csv'
        ...
        pass
    return '20200101', '20301231'  # 默认宽范围
...
def test_local_file_source():
    ...
    # 获取实际日期范围
    start_date, end_date = get_actual_date_range()
    ...
    try:
        ...
        if data.empty:
            ...
            # 尝试查看文件内容
            import os
            file_path = os.path.join(config['local_data_dir'], f"{test_symbol}.csv")
            if os.path.exists(file_path):
                df_test = pd.read_csv(file_path)
                print(f"  文件存在: {len(df_test)} 行")
                if 'date' in df_test.columns:
                    df_test['date'] = pd.to_datetime(df_test['date'])
                    print(f"  文件日期范围: {df_test['date'].min()} 到 {df_test['date'].max()}")
            return False
