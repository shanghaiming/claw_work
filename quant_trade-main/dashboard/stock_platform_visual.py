...
# 模块导入
try:
    from ..data.fetchers.fetch_daily_batch import fetch_daily_batch
    from ..data.fetchers.fetch_weekly_batch import fetch_weekly_batch
    from ..data.processors.process_batch import process_batch
...
class AuthConfig:
    ...
    USERS = {
        ...
        'user': 'user123',
        'guest': 'guest123',
        'muller718': 'zxcvbnm0717',  # 保留原始账户
        'guest_old': 'guest'  # 旧版guest账户
    }
...
class StrategyManager:
    """策略管理器 - 自动发现和管理所有BaseStrategy策略"""
    
    def __init__(self):
        self.strategies = {}  # {strategy_name: {'class': class, 'module': module, 'info': info}}
    ...
    def load_all_strategies(self):
        ...
        try:
            ...
            logger.info("✅ 成功导入BaseStrategy基类")
        except ImportError as e:
            logger.error(f"❌ 无法导入BaseStrategy: {e}")
            return
        ...
        for filename in strategy_files:
            strategy_name = filename[:-3]  # 去掉.py后缀
            ...
            try:
                ...
                if spec is None:
                    logger.warning(f"无法创建模块规范: {filename}")
                    continue
                ...
                if strategy_class:
                    ...
                    has_generate_signals = hasattr(strategy_class, 'generate_signals')
                    ...
                    strategy_info = {
                        ...
                        'description': getattr(strategy_class, '__doc__', '无描述') or '无描述',
                        'load_time': datetime.now().isoformat()
                    }
                    
                    self.strategies[strategy_name] = strategy_info
                    logger.info(f"✅ 加载策略: {strategy_name} ({class_name})")
                else:
                    logger.warning(f"⚠️  不是BaseStrategy子类: {filename}")
                    
            except Exception as e:
                logger.error(f"❌ 加载策略失败 {filename}: {e}")
        
        logger.info(f"📊 成功加载 {len(self.strategies)} 个BaseStrategy策略")
    ...
    def get_strategy_list(self):
        ...
        for name, info in self.strategies.items():
            strategy_list.append({
                'name': name,
                'display_name': info['class_name'] or name,
                'description': info['description'][:100] + '...' if len(info['description']) > 100 else info['description'],
                'has_generate_signals': info['has_generate_signals']
            })
        
        # 按名称排序
        strategy_list.sort(key=lambda x: x['name'])
    ...
    def run_strategy_stock_selection(self, strategy_name, latest_data=None, params=None):
        """
        运行策略选股
        使用回测引擎，但数据是最新的
        """
        if strategy_name not in self.strategies:
            return None, f"策略不存在: {strategy_name}"
        
        strategy_info = self.strategies[strategy_name]
        
        if not strategy_info['has_generate_signals']:
            return None, f"策略没有generate_signals方法"
        
        try:
            # 如果没有提供最新数据，使用示例数据
            if latest_data is None:
                latest_data = self.get_sample_latest_data()
            ...
            strategy_instance = strategy_class(latest_data, params)
            
            # 生成选股信号
            signals = strategy_instance.generate_signals()
            
            # 处理信号为选股结果（股票代码列表）
            stock_codes = self.process_signals_to_stock_codes(signals, latest_data)
            
            return stock_codes, "选股成功"
            
        except Exception as e:
            return None, f"选股失败: {e}"
    
    def get_sample_latest_data(self):
        """获取示例最新数据（实际应用中应该获取真实最新数据）"""
        # 创建示例数据（模拟最新数据）
        ...
        data = pd.DataFrame({
            ...
            'amount': np.random.uniform(100000, 1000000, len(dates))
        }, index=dates)
    ...
    def get_default_params(self, strategy_name):
        ...
        if 'ma' in strategy_name.lower():
            default_params.update({'short_window': 5, 'long_window': 20})
        elif 'rsi' in strategy_name.lower():
            default_params.update({'rsi_period': 14})
        
        return default_params
    
    def process_signals_to_stock_codes(self, signals, data):
        """将信号处理为股票代码列表"""
        if not signals:
            # 如果没有信号，返回一些示例股票代码
            return ['000001.SZ', '600036.SH', '601318.SH']
        
        stock_codes = []
        
        # 尝试从信号中提取股票代码
        for signal in signals[:10]:  # 最多取10个
        ...
        # 如果没有提取到股票代码，返回示例代码
        if not stock_codes:
            stock_codes = ['000001.SZ', '600036.SH', '601318.SH']
...
class StockDataManager:
    ...
    def _load_stock_data(self):
        ...
        # 数据源优先级：1. analysis_results（处理后的数据） 2. data/daily_data2（原始数据）
        project_root = Path(__file__).parent.parent
        raw_daily_path = project_root / "data" / "daily_data2"
        raw_weekly_path = project_root / "data" / "week_data2"
        ...
        for code in self.selected_codes:
            try:
                ...
                # 如果分析结果不存在，尝试从原始数据加载
                if not daily_path.exists() and raw_daily_path.exists():
                    raw_daily_file = raw_daily_path / f"{code}.csv"
                    if raw_daily_file.exists():
                        # 复制原始数据到分析结果目录
                        import shutil
                        analysis_dir = Path("analysis_results")
                        analysis_dir.mkdir(exist_ok=True)
                        shutil.copy2(raw_daily_file, daily_path)
                        print(f"📋 从原始数据复制: {code} -> {daily_path.name}")
                
                if not weekly_path.exists() and raw_weekly_path.exists():
                    raw_weekly_file = raw_weekly_path / f"{code}.csv"
                    if raw_weekly_file.exists():
                        analysis_week_dir = Path("analysis_results_week")
                        analysis_week_dir.mkdir(exist_ok=True)
                        import shutil
                        shutil.copy2(raw_weekly_file, weekly_path)
                        print(f"📋 从原始周线数据复制: {code} -> {weekly_path.name}")
    ...
    def _save_selected_codes(self):
        """保存选股结果到文件 - 与csv_auto_select.py保持一致"""
        try:
            project_root = Path(__file__).parent.parent
            output_dir = project_root / "dashboard" / "output" / "selected_stocks"
            output_dir.mkdir(parents=True, exist_ok=True)
            
            output_file = output_dir / "selected_stocks.json"
            ...
            print(f"✅ 选股结果已保存: {len(self.selected_codes)} 只股票 -> {output_file}")
        except Exception as e:
            print(f"❌ 保存选股结果失败: {e}")
...
# 策略筛选管道
def execute_strategy_pipeline(strategy_name):
    """运行BaseStrategy策略进行选股"""
    try:
        log(f"开始策略筛选任务 - {strategy_name}")
        with TaskStatus.lock:
            TaskStatus.strategy_task = True
            TaskStatus.current_strategy = strategy_name
        
        # 使用策略管理器运行选股
        selected_codes, message = strategy_manager.run_strategy_stock_selection(strategy_name)
        
        if selected_codes:
            stock_manager.update_from_auto_select(selected_codes)
            log(f"策略 {strategy_name} 筛选完成，选中 {len(selected_codes)} 只股票: {selected_codes}")
        else:
            log(f"策略 {strategy_name} 筛选失败: {message}", "error")
        
    except Exception as e:
        log(f"策略 {strategy_name} 筛选失败: {str(e)}", "error")
    finally:
        with TaskStatus.lock:
                ...
                html.Div([
                    html.Div([
                        ...
                        html.Button(
                            ...
                            id=f"strategy-btn-{strategy['name']}", 
                            className="strategy-btn",
                            title=strategy['description']
                        )
                        for strategy in strategy_manager.get_strategy_list()
                    ], className="strategy-buttons-container"),
                    html.Div([
                        html.P(f"已加载 {len(strategy_manager.strategies)} 个BaseStrategy策略", className="strategy-info"),
                        html.P("点击任意策略运行选股", className="strategy-hint")
                    ], className="strategy-status-info")
                ], className="strategy-buttons-container"),
...
@app.callback(
    ...
    [Input(f'strategy-btn-{strategy["name"]}', 'n_clicks') for strategy in strategy_manager.get_strategy_list()]
)
def update_interface(n, daily_clicks, weekly_clicks, *args):
    """数据管理页面回调 - 简化版本"""
    ...
    if ctx.triggered:
        ...
        if trigger_id in ['daily-btn', 'weekly-btn']:
            ...
            # 提取策略名称，例如从"strategy-btn-ma_strategy"中提取"ma_strategy"
            strategy_name = '-'.join(trigger_id.split('-')[2:])  # 跳过前两个部分
            threading.Thread(
