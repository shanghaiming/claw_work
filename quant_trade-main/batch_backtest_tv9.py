"""
批量回测TradingView策略 - 第九批 (Tier 2 新策略)
Traffic Light, Hurst/KFD/Entropy Regime, Epanechnikov Confluence
"""
import sys
import os
...
from strategies.traffic_light_strategy import TrafficLightStrategy
from strategies.hurst_regime_strategy import HurstRegimeStrategy
from strategies.epanechnikov_confluence_strategy import EpanechnikovConfluenceStrategy
...
STRATEGIES = {
    'TrafficLight': (TrafficLightStrategy, {}),
    'HurstRegime': (HurstRegimeStrategy, {}),
    'EpanechnikovConf': (EpanechnikovConfluenceStrategy, {}),
}
...
def run_batch():
    ...
    for strat_name, (strat_class, params) in STRATEGIES.items():
        print(f"\n{'='*80}", flush=True)
        print(f"策略: {strat_name}", flush=True)
        ...
        for symbol, name in TEST_STOCKS.items():
            try:
                ...
                if data is None or len(data) < 200:
                    print(f"  {name}({symbol}): 数据不足, 跳过", flush=True)
                    continue
                ...
                print(f"  {name}({symbol}): {total_ret:+.1f}% DD={max_dd:.1f}% "
                      f"trades={trade_count} sharpe={sharpe:.2f} win={win_rate:.1%}", flush=True)
        ...
        print(f"\n{'='*80}", flush=True)
        print("SUMMARY - Batch 9 (Tier 2 TV Strategies)", flush=True)
        print(f"{'='*80}", flush=True)
        print(f"{'Strategy':<25} {'Avg Return':>12} {'Stocks':>8} {'Profitable':>12}", flush=True)
        print("-" * 60, flush=True)
        for strat_name, r in sorted(results.items(), key=lambda x: x[1]['avg_return'], reverse=True):
            print(f"{strat_name:<25} {r['avg_return']:>+11.1f}% {r['stock_count']:>8d} "
                  f"{r['profitable']:>5d}/{r['stock_count']:<5d}", flush=True)
