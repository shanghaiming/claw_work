"""Batch 22: Remaining pre-existing strategies - Group D (5 strategies)"""
...
for mod_name, cls_name in [
    ('linreg_scalein_strategy', 'LinRegScaleInStrategy'),
    ('liquidity_sweep_strategy', 'LiquiditySweepStrategy'),
    ('macro_roc_regime_strategy', 'MacroRocRegimeStrategy'),
    ('minervini_sepa_strategy', 'MinerviniSepaStrategy'),
    ('ppo_divergence_strategy', 'PpoDivergenceStrategy'),
]:
    try:
        mod = __import__(f'strategies.{mod_name}', fromlist=[cls_name])
        sc = getattr(mod, cls_name)
        _strats.append((cls_name, sc))
