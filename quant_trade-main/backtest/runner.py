...
class BacktestRunner:
    ...
    def _discover_strategies(self, strategy_name_param=None):
        """发现策略
        Args:
            strategy_name_param: 可选，指定要加载的策略名称（如None则加载所有策略）
        Returns:
            策略字典 {策略名: 策略类}
        """
        strategies = {}
        ...
        for filename in file_list:
            ...
            try:
                ...
                # 执行模块，但重定向stdout/stderr以避免调试输出干扰
                import io
                import sys
                ...
                if found_class is None:
                    common_class_names = [
                        ...
                        "MovingAverageStrategy",  # 特殊处理ma_strategy
                ...
                if found_class is not None:
                    ...
                    if strategy_name_param is not None:  # 只在加载单个策略时显示
                        print(f"✅ 加载策略: {filename[:-3]}")
                else:
                    print(f"⚠️  策略 {filename[:-3]} 未找到继承BaseStrategy的类")
    ...
    def load_strategy(self, strategy_name):
        """加载单个策略
        Args:
            strategy_name: 策略名称（对应文件名，不含.py）
        Returns:
            策略类，如果加载失败则返回None
        """
