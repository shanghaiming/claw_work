"""
Smart_Strategy_Selector - 复合系统模式指标

# 整合适配 - 自动添加
from backtest.src.strategies.base_strategy import BaseStrategy

复合系统模式指标 #11: Smart_Strategy_Selector
基于TradingView复合系统设计思想：
1. 整合多个基础指标 (RSI, ADX, ROC, MINUS_DI)
2. 使用科学组合方法（加权、投票、分层）
3. 提供综合市场视角
4. 提高信号可靠性和稳定性
...
class TV_Smart_Strategy_Selector:
    """Smart_Strategy_Selector - 复合系统模式指标"""
    
    def __init__(self):
        """初始化复合系统指标"""
        self.name = "Smart_Strategy_Selector"
        self.description = "复合系统模式指标 #11: Smart_Strategy_Selector"
        self.author = "OpenClaw量化系统"
        self.version = "1.0.0"
        self.base_indicators = ['RSI', 'ADX', 'ROC', 'MINUS_DI']
    ...
    def calculate_individual_indicators(self, high, low, close, volume=None):
        ...
        try:
            ...
            try:
                # 尝试计算指标
                indicator_value = talib.MINUS_DI(close)
                if indicator_value is not None:
                    # 简单标准化
                    if len(indicator_value) > 0 and np.std(indicator_value) > 0:
                        results["MINUS_DI"] = (indicator_value - np.mean(indicator_value)) / np.std(indicator_value)
                    else:
                        results["MINUS_DI"] = indicator_value
            except:
                results["MINUS_DI"] = np.zeros_like(close)
    ...
    def plot_style(self):
        ...
        return {
            ...
            "components": ['RSI', 'ADX', 'ROC', 'MINUS_DI'],
            "weights": self.weights
        }

def smart_strategy_selector(high, low, close, volume=None):
    """
    简化的函数接口
    ...
    返回:
        ...
        system_health: 系统健康度指标
    """
    indicator = TV_Smart_Strategy_Selector()
