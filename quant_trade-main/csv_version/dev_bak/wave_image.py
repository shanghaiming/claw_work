...
# ========== 批处理生成所有窗口图片 ==========
def generate_all_windows_for_file(csv_path, output_dir, window_size=100, step=5):
    """
    对单个CSV文件，按窗口滑动生成所有结构分析图
    """
    try:
        df_full = pd.read_csv(csv_path, index_col='trade_date', parse_dates=['trade_date'])
        df_full = df_full.sort_index()  # 确保按时间排序
        
        if len(df_full) < window_size:
            print(f"跳过 {csv_path.name}: 数据不足 {window_size} 行")
            return
        
        # 创建输出子目录
        stock_name = csv_path.stem.replace('_analysis', '')
        stock_output_dir = Path(output_dir) / stock_name
        stock_output_dir.mkdir(parents=True, exist_ok=True)
        
        start = 0
        window_count = 0
        while start + window_size <= len(df_full):
            end = start + window_size
            df_window = df_full.iloc[start:end].copy()
            
            # 生成文件名：stock_000_0-99.png
            output_file = stock_output_dir / f"{stock_name}_{window_count:03d}_{start}-{end-1}.png"
            
            try:
                result = visualize_market_structure(df_window)
                if result is not None:
                    # 重命名已保存的文件（visualize_market_structure 默认保存为固定名）
                    default_path = Path('market_structure_analysis.png')
                    if default_path.exists():
                        default_path.rename(output_file)
                    else:
                        # 如果函数没保存，我们自己保存
                        plt.savefig(output_file, dpi=300, bbox_inches='tight')
                        plt.close()
                else:
                    print(f"  跳过窗口 {start}-{end-1}: 无有效数据")
        ...
        print(f"✅ 完成 {csv_path.name}: 生成 {window_count} 个窗口")
        
    except Exception as e:
        print(f"❌ 处理文件 {csv_path} 失败: {str(e)}")


def batch_process_all_csvs(input_dir, output_dir, window_size=100, step=5):
    """
    批量处理 input_dir 下所有 *_analysis.csv 文件
    """
    input_path = Path(input_dir)
    output_path = Path(output_dir)
    output_path.mkdir(parents=True, exist_ok=True)
    
    csv_files = list(input_path.glob("*_analysis.csv"))
    if not csv_files:
        print(f"在 {input_dir} 中未找到 *_analysis.csv 文件")
        return
    
    print(f"找到 {len(csv_files)} 个CSV文件，开始处理...")
    for csv_file in sorted(csv_files):
        print(f"\n处理文件: {csv_file.name}")
        generate_all_windows_for_file(csv_file, output_path, window_size, step)
    
    print(f"\n🎉 全部完成！图片保存至: {output_dir}")


# ========== 主程序 ==========
if __name__ == "__main__":
    # 配置路径
    input_dir = r"E:\stock\csv_version\analysis_results"  # CSV文件目录
    output_dir = r"E:\stock\csv_version\structure_images"  # 图片输出目录
    
    # 批量处理
    batch_process_all_csvs(input_dir, output_dir, window_size=100, step=5)
...
if __name__ == "__main__":
    ...
    # 批量处理
    generate_all_windows_for_file(csv_path, output_dir, window_size=100, step=5)
    #batch_process_all_csvs(input_dir, output_dir, window_size=100, step=5)