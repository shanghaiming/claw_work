...
class Config:
    ...
    OPTUNA_TRIALS = 200  # Number of Optuna trials to run
...
class EnhancedTradingModel(nn.Module):
    def __init__(self, input_dim, hidden_dim=256, output_dim=3, dropout_rate=0.3, num_layers=4):
        super().__init__()
        ...
        for i in range(num_layers - 2):
            next_dim = hidden_dim // (2 ** (i + 1)) if i < 2 else hidden_dim // 4
            layers.extend([
...
# 训练函数
def train_model(X_train, y_train, X_test, y_test, input_dim, class_weights, trial=None):
    """训练模型"""
    # 如果提供了trial，则使用Optuna建议的超参数
    if trial:
    ...
    # 创建模型
    model = EnhancedTradingModel(input_dim, hidden_dim, 3, dropout_rate, num_layers).to(Config.device)
    ...
    for epoch in range(Config.EPOCHS):
        ...
        if test_accuracy > best_accuracy:
            ...
            if not trial:  # 只在非Optuna试验时保存模型
                torch.save(model.state_dict(), Config.MODEL_PATH)
        
        # 打印进度
        if epoch % 50 == 0 and not trial:  # 在Optuna试验中减少输出
    ...
    if not trial:  # 只在非Optuna试验时绘制训练曲线
        # 绘制训练曲线
...
# Optuna目标函数
def objective(trial, X_train, y_train, X_test, y_test, input_dim, class_weights):
    """Optuna目标函数，用于超参数优化"""
    _, accuracy = train_model(X_train, y_train, X_test, y_test, input_dim, class_weights, trial)
    return accuracy
...
def main():
    ...
    study.optimize(lambda trial: objective(trial, X_train, y_train, X_test, y_test, input_dim, class_weights), 
    ...
    model, _ = train_model(X_train, y_train, X_test, y_test, input_dim, class_weights)
    ...
    best_params = {
        ...
        'weight_decay': Config.WEIGHT_DECAY,
        'batch_size': Config.BATCH_SIZE,
        'num_layers': study.best_params.get('num_layers', 4)
    }
    pd.DataFrame(best_params, index=[0]).to_csv('best_hyperparameters.csv', index=False)
    print("最佳超参数已保存到 best_hyperparameters.csv")
