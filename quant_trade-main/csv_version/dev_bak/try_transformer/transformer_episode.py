...
class Config:
    ...
    # 保存路径
    MODEL_PATH = f'grpo_trading_model_{datetime.now().strftime("%Y%m%d")}_episode.pth'
...
class TradingEnv:
    ...
    def reset(self):
        ...
        self.stock_value = Config.INITIAL_CAPITAL  # 初始满仓
    ...
    def calculate_reward(self, portfolio_value, action, prev_position, market_return):
        ...
        if volatility < 1e-5:  # 避免除零错误
            volatility = 1e-5
            
        excess_return = portfolio_return - (Config.RISK_FREE_RATE / 252)
        risk_adjusted_reward = excess_return / (volatility + 1e-5)
        
        # 3. 交易成本惩罚
        trade_penalty = 0
        if action != 1 and prev_position != self.position:  # 实际执行了交易
            trade_penalty = -0.005  # 固定交易惩罚
        
        # 4. 方向一致性奖励 (权重降低)
        direction_reward = 0
        if (self.position == 1 and market_return > 0) or (self.position == 0 and market_return < 0):
            direction_reward = 0.1 * abs(market_return)
            self.consecutive_correct += 1
        else:
            self.consecutive_correct = 0
        
        # 5. 连续正确奖励
        consecutive_bonus = 0.01 * min(self.consecutive_correct, 5)  # 最多奖励0.05
        
        # 6. 波动率惩罚
        volatility_penalty = -0.1 * volatility * self.position
        
        # 组合奖励 (风险调整收益为主)
        reward = risk_adjusted_reward + trade_penalty + direction_reward + consecutive_bonus + volatility_penalty
...
def main():
    ...
    # 移除不必要的列
    df = df.drop(columns=['circ_mv', 'support', 'resistance', 'ts_code', 'action'], errors='ignore')
    ...
    # 2. 特征工程和选择
    all_features = [col for col in df.columns if col not in ['Date', 'returns', 'volatility']]
    ...
    if not agent.load_model(Config.MODEL_PATH):
        ...
        for epoch in range(Config.EPOCHS):
            ...
            while not done:
                ...
                # 定期更新不确定性估计器
                if step_count % 10 == 0 and len(agent.episode_states) >= 10:
                    states = np.array(agent.episode_states[-100:])
                    rewards = np.array(agent.episode_rewards[-100:])
