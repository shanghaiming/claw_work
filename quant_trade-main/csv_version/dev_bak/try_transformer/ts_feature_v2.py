...
class Hyperparameters:
    def __init__(self):
        ...
        # 损失函数参数
        self.loss_type = 'focal' #'cross_entropy'
        self.focal_alpha = 0.25
        self.focal_gamma = 2.0
        
        # 早期淘汰参数
        self.early_rejection_threshold = 0.001  # 训练损失下降率阈值
        self.min_epochs_before_reject = 5       # 至少训练这么多epoch后再考虑淘汰
...
class StockDataset(Dataset):
    def __init__(self, features, targets, window_size=50):
...
# 准备单只股票数据
def prepare_single_stock_data(file_path, hp):
    ...
    # 创建数据集
    train_dataset = StockDataset(train_features, train_targets, hp.window_size)
    test_dataset = StockDataset(test_features, test_targets, hp.window_size)
    ...
    return train_loader, test_loader, class_weights, len(feature_columns), len(class_counts)
...
def objective(trial):
    ...
    hp.window_size = trial.suggest_int('window_size', 10, 100, step=10)
    ...
    train_loader, val_loader, class_weights, num_features, num_classes = prepare_single_stock_data(file_path, hp)
...
def main():
    ...
    if not args.skip_optuna:
        ...
        # 使用lambda函数传递logger
        study.optimize(lambda trial: objective(trial), n_trials=args.trials)
        ...
        # 使用预定义的最佳参数
        best_params = {'d_model': 64, 'nhead': 16, 'num_layers': 3, 'dim_feedforward': 256, 'dropout': 0.009659433619514284, 'learning_rate': 0.0003499079840088863, 
                       'window_size': 80, 'optimizer_type': 'adamw', 'weight_decay': 0.0014021554099022194}
