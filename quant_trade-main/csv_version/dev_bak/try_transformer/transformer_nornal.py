...
class Config:
    ...
    # 保存路径
    MODEL_PATH = f'grpo_trading_model_{datetime.now().strftime("%Y%m%d")}_reward_explore_optmem.pth'
    #MODEL_PATH = f'grpo_trading_model_20250820_reward_explore_optmem.pth'
    
    # 特征选择
    FEATURE_SELECTION_K = 10  # 选择最重要的K个特征

def group_based_normalization(data, feature_names, group_config):
    """
    基于分组的标准化函数
    
    参数:
    data: 输入数据 (2D数组)
    feature_names: 所有特征名称的列表
    group_config: 分组配置字典
        {
            'group_a': {
                'method': 'cross',
                'ref_col': 'reference_column_name',
                'columns': ['col1', 'col2', ...]  # 属于该组的列名
            },
            'group_b': {
                'method': 'cross', 
                'ref_col': 'reference_column_name',
                'columns': ['col3', 'col4', ...]  # 属于该组的列名
            },
            # 其余列将使用StandardScaler
        }
    ...
    for group_name, config in group_config.items():
        method = config.get('method', 'cross')
        ref_col = config.get('ref_col')
        columns = config.get('columns', [])
        
        # 验证参考列是否存在
        if ref_col and ref_col not in feature_names:
            raise ValueError(f"参考列 '{ref_col}' 未找到")
        
        # 计算参考列的统计量（如果需要）
        if method == 'cross' and ref_col:
            ref_col_idx = feature_names.index(ref_col)
            ref_mean = np.mean(data[:, ref_col_idx])
            ref_std = np.std(data[:, ref_col_idx])
        ...
        for col_name in columns:
            ...
            if method == 'cross' and ref_col:
                # 跨列标准化
                normalized_data[:, col_idx] = (data[:, col_idx] - ref_mean) / ref_std
            else:
                # 默认使用StandardScaler
                scaler = StandardScaler()
                normalized_data[:, col_idx] = scaler.fit_transform(data[:, col_idx].reshape(-1, 1)).flatten()
            
            # 标记该列已被处理
            processed_columns[col_name] = True
    
    # 处理未分组的列（使用StandardScaler）
...
class TradingEnv:
    ...
    def get_state(self):
        ...
        group_config = {
            'group_a': {
                'method': 'cross',
                'ref_col': 'close',  # 使用收盘价作为参考
                'columns': ['low',  'high', 'open', 'ma5', 'ma8', 'ma13', 'ma21', 'ma34', 'ma55', 'tsma5', 'tsma8', 'tsma13', 'sma', 'std', 
                            'upper', 'lower','target']  
            },
            'group_b': {
                'method': 'cross',
                'ref_col': 'macd',  # 使用成交量作为参考
            '   columns': ['diff', 'dea']  # B组特征
            },
            'group_c': {
                'method': 'cross',
                'ref_col': 'wr5',  # 使用成交量作为参考
            '   columns': ['wr55']  # B组特征
            }
            # 其余特征将使用StandardScaler
        }
    
        # 应用分组标准化
        normalized_state = group_based_normalization(
            state, 
...
def main():
    ...
    # 特征选择 (只在训练集上进行)
    Config.feature_cols = all_features
