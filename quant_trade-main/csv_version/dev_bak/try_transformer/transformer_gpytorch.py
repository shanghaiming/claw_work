...
class Config:
    ...
    print(f"使用设备: {device}")
    print(torch.cuda.is_available())  # 期望True
    print(torch.cuda.device_count())   # 期望大于0
    print(torch.cuda.get_device_name(0)) # 期望输出显卡型号
    # 保存路径
    MODEL_PATH = f'grpo_trading_model_20250811_gp.pth'
...
class GRPOAgent:
    def __init__(self, feature_extractor, state_dim, action_dim, config):
        ...
        self.likelihood = gpytorch.likelihoods.GaussianLikelihood().to(config.device)
        self.gp_model = None
    ...
    def act(self, state):
        # 使用固定内存减少传输开销
        state_tensor = torch.as_tensor(state, dtype=torch.float32, device=self.config.device).unsqueeze(0)
    ...
    def update_gp(self):
        ...
        train_x = features
        train_y = torch.FloatTensor(rewards).to(self.config.device)
        
        if self.gp_model is None:
            self.likelihood = gpytorch.likelihoods.GaussianLikelihood().to(self.config.device)
            self.gp_model = GPModel(train_x, train_y, self.likelihood).to(self.config.device)
        else:
...
class Backtester:
    ...
    def run_backtest(self):
        ...
        pd.DataFrame(self.results_r).to_csv('trading_results_tf.csv', index=False)
        return self.results
    ...
    if not agent.load_model(Config.MODEL_PATH):
        ...
        for epoch in range(Config.EPOCHS):
            ...
            while not done:
                ...
                # 执行动作
                next_state, reward, done, portfolio_value_n = train_env.step(action)
                total_reward += reward
                step_count += 1
            ...
            # 打印进度
            print(f"Epoch {epoch+1}/{Config.EPOCHS}, 总奖励: {total_reward:.2f}, 组合价值: ${portfolio_value_n:,.2f}, 探索率: {agent.epsilon:.3f}")
        # 绘制训练奖励曲线
