...
# To display minus signs correctly
...
def get_trade_days(start, end):
    ...
    a = (
    pd.to_datetime(df['pretrade_date'], format='%Y%m%d')
    .dt.strftime('%Y%m%d')
    .to_list()
)
    return a
...
def select(stock, date):
    try:
        ...
        #    if result_df['divergence_status'].iloc[-1] == "confirmed":
                #errors.append("背离"   )    
        if hist['divergence_status_vr'].iloc[-1] == "confirmed" or hist['divergence_status_macd'].iloc[-1] == "confirmed" or hist['divergence_status_wr5'].iloc[-1] == "confirmed":
            errors.append("背离"   ) 
        # 均线极性筛选
        ...
        if hist['ma_trend_change_win1'].iloc[-1] or hist['ma_trend_change_win2'].iloc[-1]:
            errors.append("均线"      )  
        # 均线交叉筛选
        #if detect_cross_events(hist, window_size=13) >= 1:
        if hist['cross_events'].iloc[-1] >=1:
            errors.append( "均线交叉"  )      
        # 高低点检测
...
def main(start_date, end_date):
    ...
    # 1. 未通过原因分布
    plt.subplot(2, 2, 1)
    reason_counts = df['reason'].astype(str).str.split(',').explode().value_counts()
    print(reason_counts)
