...
WEEK_DATA_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'week_data2')   # 存放日线数据的文件夹
EXCHANGE = 'SSE'             # 交易所代码（上海主板）

#BATCH_SIZE = 1             # 每个线程处理的股票数量 初次
BATCH_SIZE = 2000             # 每个线程处理的股票数量 增量 <6000/time interval
THREAD_POOL_SIZE = 1         # 线程池大小（根据网络环境调整）

# 新增常量定义
LAST_RUN_FILE = os.path.join(WEEK_DATA_DIR, 'last_run_date.txt')  # 存储最后运行日期的文件路径
...
def fetch_data(pro, ts_codes, start_date_str, end_date_str):
    ...
    try:
        ...
        while current_end <= end_date:
            ...
            batch_end = min(current_end + timedelta(weeks=6000), end_date)
            batch_end_str = batch_end.strftime('%Y%m%d')
                
            # 统计API请求耗时
            api_start = time.time()
            df = pro.weekly(ts_code=','.join(ts_codes), 
...
def save_data(df, data_dir):
    try:
        ...
        logger.error(f"[ERROR] save_data failed: {e}", file=sys.stderr)
        raise EXCEPTION
...
def process_stocks(pro, stock_list, data_dir):
    ...
    if last_run_date:
        ...
        # 首次运行，获取1000天数据
        start_date = (datetime.strptime(end_date_str, '%Y%m%d') - timedelta(days=8000)).strftime('%Y%m%d')
...
def fetch_weekly_batch():
    ...
    try:
        process_stocks(pro, stock_list, WEEK_DATA_DIR)
        return True
    except Exception as e:
        logger.error(f"[ERROR] 主程序异常: {e}")
        raise EXCEPTION  (f"[ERROR] 主程序异常: {e}")  
