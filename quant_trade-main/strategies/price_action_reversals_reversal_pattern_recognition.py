...
class ReversalPatternRecognition(BaseStrategy):
    """
    反转模式识别量化分析系统
    ...
    def __init__(self, data: pd.DataFrame, params: Dict):
        ...
        # 系统配置
        self.config = {
            "min_pattern_confidence": params.get('min_pattern_confidence', 0.6),      # 最小模式置信度
            "min_pattern_strength": params.get('min_pattern_strength', 60.0),       # 最小模式强度（0-100）
            "default_risk_per_trade": params.get('default_risk_per_trade', 0.02),     # 默认每笔交易风险（2%）
            "min_risk_reward_ratio": params.get('min_risk_reward_ratio', 1.5),       # 最小风险回报比
            "neckline_tolerance": params.get('neckline_tolerance', 0.01),         # 颈线容忍度（1%）
            "pattern_completion_threshold": params.get('pattern_completion_threshold', 0.7), # 模式完成阈值
            "volume_confirmation_required": params.get('volume_confirmation_required', True), # 是否需要成交量确认
        }
    ...
    def generate_signals(self) -> List[Dict]:
        ...
        high_confidence_patterns = [
            p for p in patterns 
            if p.confidence_score >= self.config["min_pattern_confidence"] 
            and p.pattern_strength >= self.config["min_pattern_strength"]
        ]
        
        # 转换为交易信号
        for pattern in high_confidence_patterns:
            # 确定交易方向
            if pattern.pattern_type.value in ["double_bottom", "head_shoulders_bottom", "triple_bottom", "rounding_bottom", "falling_wedge"]:
                action = "buy"
            else:
                action = "sell"
            
            # 计算入场价格（颈线突破价格）
            entry_price = pattern.neckline if pattern.neckline else pattern.points[0].price
            
            signal = {
                'timestamp': pattern.detected_time,
                'action': action,
                'price': entry_price,
                'confidence': pattern.confidence_score,
                'pattern_type': pattern.pattern_type.value,
                'target_price': pattern.target_price,
                'risk_reward_ratio': (pattern.target_price - entry_price) / abs(pattern.target_price - entry_price) if pattern.target_price != entry_price else 1.0
            }
