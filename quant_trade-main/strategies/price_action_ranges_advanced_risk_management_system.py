...
# ============================================================================
# 策略改造: 添加PriceActionRangesAdvancedRiskManagementSystemStrategy类
# 将价格行为区间高级风险管理系统转换为交易策略
# ============================================================================

class PriceActionRangesAdvancedRiskManagementSystemStrategy(BaseStrategy):
    """价格行为区间高级风险管理策略"""
    ...
    def generate_signals(self):
        ...
        # 分析风险评估
        overall_assessment = risk_report.get('overall_risk_assessment', {})
        risk_level = overall_assessment.get('risk_level', '').lower()
        ...
        # 根据风险分析生成信号
        if risk_level in ['low', 'very low'] and risk_score >= 70:
            # 低风险，高分数，买入信号
            self._record_signal(
        ...
        elif risk_level in ['high', 'very high'] or risk_score <= 30:
            # 高风险或低分数，卖出信号
            self._record_signal(
                timestamp=self.data.index[-1],
                action='sell',
