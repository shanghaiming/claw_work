...
# ============================================================================
# 策略改造: 添加PriceActionRangesMarketStructureIdentifierStrategy类
# 将价格行为区间市场结构识别系统转换为交易策略
# ============================================================================

class PriceActionRangesMarketStructureIdentifierStrategy(BaseStrategy):
    """价格行为区间市场结构识别策略"""
    ...
    def generate_signals(self):
        ...
        """
        # 识别市场结构
        structure_result = self.identifier.identify_market_structure(self.data)
        
        # 分析市场结构类型
        structure_type = structure_result.get('structure_type', {})
        market_type = structure_type.get('type', '').lower()
        
        # 获取结构强度
        current_strength = structure_result.get('current_strength', 0)
        
        # 根据市场结构生成信号
        if market_type == 'trend' and current_strength >= 0.7:
            # 强趋势市场，买入信号
            self._record_signal(
        ...
        elif market_type == 'range' and current_strength >= 0.6:
            # 强区间市场，卖出信号（区间交易）
            self._record_signal(
                timestamp=self.data.index[-1],
                action='sell',
