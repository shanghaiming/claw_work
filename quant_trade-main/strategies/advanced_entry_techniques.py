...
# ============================================================================
# 策略改造: 添加AdvancedEntryTechniquesStrategy类
# 将高级入场技术分析器转换为交易策略
# ============================================================================

class AdvancedEntryTechniquesStrategy(BaseStrategy):
    """高级入场技术策略"""
    ...
    def __init__(self, data: pd.DataFrame, params: dict):
        ...
        # 创建高级入场技术分析器实例
        self.techniques = AdvancedEntryTechniques()
    ...
    def generate_signals(self):
        ...
        """
        # 分析入场设置
        analysis_result = self.techniques.analyze_entry_setups(self.data)
        
        # 获取入场信号
        entry_signals = analysis_result.get('signals', [])
        ...
        for signal in entry_signals:
            ...
            if action in ['buy', 'long']:
                self._record_signal(
                    ...
                    action='buy',
                    price=signal.get('suggested_price', self.data['close'].iloc[-1])
                )
        ...
        if not self.signals:
            ...
            if optimal_entry.get('optimal_entry', False):
                entry_type = optimal_entry.get('entry_type', '').lower()
                if 'buy' in entry_type or 'long' in entry_type:
                    self._record_signal(
                        timestamp=self.data.index[-1],
                        action='buy',
                        price=optimal_entry.get('suggested_price', self.data['close'].iloc[-1])
                    )
