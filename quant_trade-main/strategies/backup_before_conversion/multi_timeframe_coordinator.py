...
# ============================================================================
# 策略改造: 添加MultiTimeframeCoordinatorStrategy类
# 将多时间框架协调器转换为交易策略
# ============================================================================

class MultiTimeframeCoordinatorStrategy(BaseStrategy):
    """多时间框架协调策略"""
    ...
    def __init__(self, data: pd.DataFrame, params: dict):
        ...
        timeframe_weights = params.get('timeframe_weights', 
            {'D': 0.25, 'W': 0.25, 'H4': 0.25, 'H1': 0.25})
        consistency_threshold = params.get('consistency_threshold', 0.65)
        
        # 创建多时间框架协调器实例
        self.coordinator = MultiTimeframeCoordinator(
    ...
    def generate_signals(self):
        ...
        基于多时间框架协调生成交易信号
        """
        # 注意: 多时间框架协调器需要多个时间框架的数据
        # 这里使用单一数据源模拟多时间框架
        # 在实际使用中，需要提供多个时间框架的数据
        
        # 模拟多时间框架数据 (从当前数据生成)
        timeframes_data = {
            'D': self.data.iloc[::1],    # 日线
            'H4': self.data.iloc[::6],   # 4小时 (每6个数据点)
            'H1': self.data.iloc[::24],  # 1小时 (每24个数据点)
        }
        
        # 调用多时间框架协调分析
        analysis_result = self.coordinator.analyze_timeframe_alignment(timeframes_data)
        
        # 获取共识信号
        consensus_signal = analysis_result.get('consensus_signal', {})
        signal = consensus_signal.get('signal', 'hold').lower()
        confidence = consensus_signal.get('confidence', 0)
        
        # 将共识信号转换为交易信号
        if signal in ['buy', 'strong_buy'] and confidence > 0.6:
            self._record_signal(
                timestamp=self.data.index[-1],
                action='buy',
                price=self.data['close'].iloc[-1]
            )
        elif signal in ['sell', 'strong_sell'] and confidence > 0.6:
            self._record_signal(
                timestamp=self.data.index[-1],
                action='sell',
                price=self.data['close'].iloc[-1]
