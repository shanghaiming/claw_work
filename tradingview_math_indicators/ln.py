...
class TV_LN:
    """TradingView风格的LN数学变换指标"""
    
    def __init__(self):
        """初始化指标"""
        self.name = "LN"
        self.description = "自然对数函数，计算以e为底的对数"
    ...
    def signal(self, real, threshold=0.5):
        ...
        try:
            ...
            elif "LN" in ["SIN", "COS", "TAN", "SINH", "COSH", "TANH"]:
                # 三角函数，检测周期性变化
                for i in range(1, len(values)):
                    if values[i] > threshold and values[i-1] <= threshold:
                        signal[i] = -1  # 达到峰值
...
if __name__ == "__main__":
    ...
    # 特殊处理某些函数
    if "LN" in ["SQRT", "LN", "LOG10"]:
        real = np.random.uniform(0.1, 10, n)  # 正数
    elif "LN" in ["ACOS", "ASIN"]:
