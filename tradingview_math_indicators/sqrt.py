"""
TV_SQRT - TradingView风格的SQRT数学变换指标
...
class TV_SQRT:
    """TradingView风格的SQRT数学变换指标"""
    
    def __init__(self):
        """初始化指标"""
        self.name = "SQRT"
        self.description = "平方根函数，计算非负实数的平方根"
    ...
    def signal(self, real, threshold=0.5):
        ...
        try:
            ...
            elif "SQRT" in ["SIN", "COS", "TAN", "SINH", "COSH", "TANH"]:
                # 三角函数，检测周期性变化
                for i in range(1, len(values)):
                    if values[i] > threshold and values[i-1] <= threshold:
                        signal[i] = -1  # 达到峰值
    ...
    def plot_style(self):
        ...
        color = color_map.get("SQRT", "#2962FF")
...
if __name__ == "__main__":
    ...
    # 特殊处理某些函数
    if "SQRT" in ["SQRT", "LN", "LOG10"]:
        real = np.random.uniform(0.1, 10, n)  # 正数
    elif "SQRT" in ["ACOS", "ASIN"]:
