"""
TV_stream_COS - TradingView风格的stream_COS数学变换指标

余弦函数，计算角度的余弦值 (流式版本，支持实时更新)
基于TA-Lib的stream_COS函数实现
...
class TV_stream_COS:
    """TradingView风格的stream_COS数学变换指标"""
    
    def __init__(self):
        """初始化指标"""
        self.name = "stream_COS"
        self.description = "余弦函数，计算角度的余弦值 (流式版本，支持实时更新)"
    ...
    def signal(self, real, threshold=0.5):
        ...
        try:
            ...
            elif "COS" in ["SIN", "COS", "TAN", "SINH", "COSH", "TANH"]:
                # 三角函数，检测周期性变化
                for i in range(1, len(values)):
                    if values[i] > threshold and values[i-1] <= threshold:
                        signal[i] = -1  # 达到峰值
    ...
    def plot_style(self):
        ...
        color = color_map.get("COS", "#2962FF")
...
if __name__ == "__main__":
    ...
    # 特殊处理某些函数
    if "COS" in ["SQRT", "LN", "LOG10"]:
        real = np.random.uniform(0.1, 10, n)  # 正数
    elif "COS" in ["ACOS", "ASIN"]:
