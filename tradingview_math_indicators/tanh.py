"""
TV_TANH - TradingView风格的TANH数学变换指标
...
class TV_TANH:
    """TradingView风格的TANH数学变换指标"""
    
    def __init__(self):
        """初始化指标"""
        self.name = "TANH"
        self.description = "双曲正切函数"
    ...
    def signal(self, real, threshold=0.5):
        ...
        try:
            ...
            elif "TANH" in ["SIN", "COS", "TAN", "SINH", "COSH", "TANH"]:
                # 三角函数，检测周期性变化
                for i in range(1, len(values)):
                    if values[i] > threshold and values[i-1] <= threshold:
                        signal[i] = -1  # 达到峰值
    ...
    def plot_style(self):
        ...
        color = color_map.get("TANH", "#2962FF")
...
if __name__ == "__main__":
    ...
    # 特殊处理某些函数
    if "TANH" in ["SQRT", "LN", "LOG10"]:
        real = np.random.uniform(0.1, 10, n)  # 正数
    elif "TANH" in ["ACOS", "ASIN"]:
