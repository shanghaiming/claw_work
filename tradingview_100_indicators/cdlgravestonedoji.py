"""
CDLGRAVESTONEDOJI - 蜡烛图形态识别指标

基于TA-Lib的CDLGRAVESTONEDOJI函数实现
...
class TV_CDLGRAVESTONEDOJI:
    """TradingView风格的CDLGRAVESTONEDOJI蜡烛图形态指标"""
    
    def __init__(self):
        """初始化指标"""
        self.name = "CDLGRAVESTONEDOJI"
        self.description = "蜡烛图形态识别: CDLGRAVESTONEDOJI"
        self.author = "OpenClaw量化系统"
        self.version = "1.0.0"
        
    def calculate(self, open, high, low, close):
        """
        计算CDLGRAVESTONEDOJI形态
        ...
        """
        try:
            result = talib.CDLGRAVESTONEDOJI(open, high, low, close)
            return result
...
def cdlgravestonedoji(open, high, low, close):
    ...
    返回:
        result: 形态识别结果
    """
    indicator = TV_CDLGRAVESTONEDOJI()
