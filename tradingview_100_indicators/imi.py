...
class TV_IMI:
    ...
    def __init__(self):
        """初始化指标"""
        self.name = "IMI"
        self.description = "基于TA-Lib IMI函数的TradingView风格实现"
        self.author = "OpenClaw量化系统"
        self.version = "1.0.0"
        
    def calculate(self, open, close, timeperiod):
        """
        计算IMI指标
        ...
        """
        try:
            result = talib.IMI(open, close, timeperiod)
            return result
        except Exception as e:
            print(f"计算指标{self.name}时出错: {e}")
            return None
    
    def signal(self, open, close, timeperiod, threshold=0):
        """
        生成交易信号
        ...
        """
        try:
            values = self.calculate(open, close, timeperiod)
            if values is None:
...
def imi(open, close, timeperiod):
    ...
    """
    indicator = TV_IMI()
    return indicator.calculate(open, close, timeperiod)
...
if __name__ == "__main__":
    ...
    # 测试计算
    try:
        result = indicator.calculate(open, close, timeperiod)
        ...
        # 测试信号生成
        signal = indicator.signal(open, close, timeperiod, threshold=0)
