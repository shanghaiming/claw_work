"""
CDLGAPSIDESIDEWHITE - 蜡烛图形态识别指标

基于TA-Lib的CDLGAPSIDESIDEWHITE函数实现
...
class TV_CDLGAPSIDESIDEWHITE:
    """TradingView风格的CDLGAPSIDESIDEWHITE蜡烛图形态指标"""
    
    def __init__(self):
        """初始化指标"""
        self.name = "CDLGAPSIDESIDEWHITE"
        self.description = "蜡烛图形态识别: CDLGAPSIDESIDEWHITE"
        self.author = "OpenClaw量化系统"
        self.version = "1.0.0"
        
    def calculate(self, open, high, low, close):
        """
        计算CDLGAPSIDESIDEWHITE形态
        ...
        """
        try:
            result = talib.CDLGAPSIDESIDEWHITE(open, high, low, close)
            return result
...
def cdlgapsidesidewhite(open, high, low, close):
    ...
    返回:
        result: 形态识别结果
    """
    indicator = TV_CDLGAPSIDESIDEWHITE()
